...
public class FlutterVmServiceListener implements VmServiceListener {
  private static final Logger LOG = Logger.getInstance(FlutterVmServiceListener.class.getName());

  @NotNull private final FlutterVmServiceDebugProcess myDebugProcess;
  @NotNull private final FlutterVmServiceBreakpointHandler myBreakpointHandler;
  @Nullable private XSourcePosition myLatestSourcePosition;

  public FlutterVmServiceListener(@NotNull final FlutterVmServiceDebugProcess debugProcess,
  ...
  void onIsolatePaused(@NotNull final IsolateRef isolateRef,
    ...
    final FlutterVmServiceSuspendContext suspendContext =
      new FlutterVmServiceSuspendContext(myDebugProcess, isolateRef, vmTopFrame, exception, atAsyncSuspension);
  ...
  private String evaluateExpression(final @NotNull String isolateId,
    ...
    myDebugProcess.getVmServiceWrapper().evaluateInFrame(isolateId, vmTopFrame, evalText, new XDebuggerEvaluator.XEvaluationCallback() {
      ...
      public void evaluated(@NotNull final XValue result) {
        if (result instanceof FlutterVmServiceValue) {
          evalResult.set(getSimpleStringPresentation(((FlutterVmServiceValue)result).getInstanceRef()));
        }
        semaphore.up();
      }
