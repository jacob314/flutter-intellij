...
public class FlutterVmServiceWrapper implements Disposable {

  public static final Logger LOG = Logger.getInstance(FlutterVmServiceWrapper.class.getName());
  private static final long RESPONSE_WAIT_TIMEOUT = 3000; // millis

  private final FlutterVmServiceDebugProcess myDebugProcess;
  private final VmService myVmService;
  private final FlutterVmServiceListener myVmServiceListener;
  private final IsolatesInfo myIsolatesInfo;
  private final FlutterVmServiceBreakpointHandler myBreakpointHandler;
  ...
  public FlutterVmServiceWrapper(@NotNull final FlutterVmServiceDebugProcess debugProcess,
                                 @NotNull final VmService vmService,
                                 @NotNull final FlutterVmServiceListener vmServiceListener,
                                 @NotNull final IsolatesInfo isolatesInfo,
                                 @NotNull final FlutterVmServiceBreakpointHandler breakpointHandler) {
  ...
  public void handleIsolate(@NotNull final IsolateRef isolateRef, final boolean isolatePausedStart) {
    ...
    // Just to make sure that the main isolate is not handled twice, both from handleDebuggerConnected() and FlutterVmServiceListener.received(PauseStart)
    if (newIsolate) {
      final ExceptionPauseMode mode = FlutterExceptionBreakpointHandler
        .getBreakOnExceptionMode(myDebugProcess.getSession(),
                                 FlutterExceptionBreakpointHandler.getDefaultExceptionBreakpoint(myDebugProcess.getSession().getProject()));
        ...
        ApplicationManager.getApplication().executeOnPooledThread(() -> {
          ...
          for (final Frame vmFrame : vmFrames) {
            if (vmFrame.getKind() == FrameKind.AsyncSuspensionMarker) {
              ...
              final XStackFrame markerFrame = new FlutterAsyncMarkerFrame();
              xStackFrames.add(markerFrame);
            }
            else {
              final FlutterVmServiceStackFrame stackFrame =
                new FlutterVmServiceStackFrame(myDebugProcess, isolateId, vmFrame, vmFrames, exceptionToAddToFrame);
  ...
  public void evaluateInFrame(@NotNull final String isolateId,
    ...
    addRequest(() -> myVmService.evaluateInFrame(isolateId, vmFrame.getIndex(), expression, new EvaluateInFrameConsumer() {
      ...
      public void received(ErrorRef errorRef) {
        callback.errorOccurred(FlutterVmServiceEvaluator.getPresentableError(errorRef.getMessage()));
      }
