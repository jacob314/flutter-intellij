...
public class PropertyEditorPanel extends JBPanel implements Disposable {
  ...
  int getOffset() {
   return Math.min(marker.getStartOffset(), document.getTextLength());
  }
  ...
  public PropertyEditorPanel(@Nullable InspectorService inspectorService,
    ...
    int offset = Math.min(position.getOffset(), documentLength);
    int endOffset = Math.min(offset + 1, documentLength);

    marker = document.createRangeMarker(offset, endOffset);
    lookupWidgetDescription();

    MigLayout manager = new MigLayout("insets 0", // Layout Constraints
                                      "[::120]5[:150:300]", // Column constraints
                                      "[]0[]");
    setLayout(manager);
...
///          field = new JBTextField(property.getExpression());
  ...
  public static Balloon showPopup(InspectorService inspectorService,
                                  ...
                                  Point point) {
    final Balloon balloon = showPopupHelper(inspectorService, editor.getProject(), editor.getDocument(), node, position, service);
    if (position != null) {
  ...
  public static Balloon showPopup(InspectorService inspectorService,
                                  ...
                                  FlutterDartAnalysisServer service,
                                  Point point) {
    final Balloon balloon = showPopupHelper(inspectorService, project, document, node, position, service);
 ...
 //   assert (node != null || position != null);
    final Color GRAPHITE_COLOR = new JBColor(new Color(200, 200, 200, 170), new Color(100, 100, 100, 170));

    PropertyEditorPanel panel =
      new PropertyEditorPanel(inspectorService, project, document, node, position, service);
  ...
  private void setPropertyValue(String propertyName, FlutterWidgetPropertyValue value) {
    if (Objects.equals(value.getExpression(), "")) {
      // treat empty expressions as null.
      value = null;
    }
    System.out.println("XXX set " + propertyName+ ", " + value);
    lookupWidgetDescription();
    ...
    if (Objects.equals(property.getValue(), value)) {
      System.out.println("XXX skipping as unchanged");
