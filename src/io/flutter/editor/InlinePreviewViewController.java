 ...
 */
public class InlinePreviewViewController extends PreviewViewControllerBase implements CustomHighlighterRenderer, EditorPositionService.Listener {
  public InlinePreviewViewController(InlineWidgetViewModelData data, boolean drawBackground) {
    super(data, drawBackground);
    if (getEditor() != null) {
      data.context.editorPositionService.addListener(getEditor(), this);
    }
 ...
 */
  ...
  public void dispose() {
    if (getEditor() != null) {
      data.context.editorPositionService.removeListener(getEditor(), this);
    }
  ...
  public boolean isValid() {
    if (getEditor() == null || !getEditor().isDisposed()) return true;
    // TODO(jacobr): eagerly disposing here is a bit odd. This indicates we
    // didin't understand how to dispose stuff.
    dispose();
    return false;
  }
  ...
  protected void showPopup(Point location, DiagnosticsNode node) {
    ...
    popup = PropertyEditorPanel
      .showPopup(data.context.inspectorStateService, getEditor(), node, node.getCreationLocation().getLocation(), data.context.flutterDartAnalysisService, location);
  }
