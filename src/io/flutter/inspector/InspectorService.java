...
public class InspectorService implements Disposable {
  ...
  CompletableFuture<DiagnosticsNode> parseDiagnosticsNode(InstanceRef instanceRef, InspectorObjectGroup objectGroup) {
    return instanceRefToJson(instanceRef).thenApplyAsync((JsonElement jsonElement) -> {
      ...
      return new DiagnosticsNode(jsonElement.getAsJsonObject(), this, objectGroup, false);
    });
  }
  ...
  CompletableFuture<ArrayList<DiagnosticsNode>> parseDiagnosticsNodes(InstanceRef instanceRef, InspectorObjectGroup objectGroup) {
    return instanceRefToJson(instanceRef).thenApplyAsync((JsonElement jsonElement) -> {
      ...
      for (JsonElement element : jsonArray) {
        nodes.add(new DiagnosticsNode(element.getAsJsonObject(), this, objectGroup, false));
      }
      return nodes;
    });
  }
  ...
  CompletableFuture<ArrayList<DiagnosticsNode>> getChildren(InspectorInstanceRef instanceRef, boolean summaryTree, InspectorObjectGroup objectGroup) {
    if (summaryTree) {
      return getListHelper(instanceRef, "getChildrenSummaryTree", objectGroup);
    } else {
  ...
  public CompletableFuture<DiagnosticsNode> getRootWidget(InspectorObjectGroup objectGroup) {
    if (isDetailsSummaryViewSupported()) {
      return parseDiagnosticsNode(invokeServiceMethod("getRootWidgetSummaryTree", objectGroup), objectGroup);
    } else {
