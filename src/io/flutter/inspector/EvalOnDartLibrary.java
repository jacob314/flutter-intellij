...
public class EvalOnDartLibrary implements Disposable {
  ...
  CompletableFuture<?> allPendingRequestsDone;
  final Object pendingRequestLock = new Object();

  /**
   * Public so that other classes can ensure their requests are in a consistent
   * order with requests which eliminates otherwise painful timing bugs such as
   * if a request to dispose an InspectorService.ObjectGroup was issued after a
   * request to read properties from an object in a group but the request to
   * dispose the object group occurred first.
   *
   * The design is we have at most 1 pending request at a time. This sacrifices
   * throughput with the advantage of predictable semantics and the benefit
