...
public class TreeScrollAnimator implements Disposable {
  ...
  private void computeScrollPosition() {
    scrollPosition = new Point(scrollbars[JScrollBar.HORIZONTAL].getModel().getValue(), scrollbars[JScrollBar.VERTICAL].getModel().getValue());
  }
  ...
  private void verticalScrollChanged(ChangeEvent e) {
    handleScrollChanged();
    if (targets != null || scrollTriggeredByCode) {
      return;
    }
    if (autoHorizontalScroll) {
      applyAutoHorizontalScroll();
  ...
  private int calculateTargetX(Point candidate, TreePath selectionPath) {
    ...
    if (selectedBounds != null) {
      ...
      int selectionWidth =  Math.min(scrollPane.getViewport().getWidth() / 2, 100);
   ...
   * and includes the required and if possible as much content from
   * the ideal interval as well.
   * <p>
   * The required interval must not be outside the ideal interval.
   */
  ...
  public void animateTo(List<TreePath> targets) {
    ...
    tree.rawScrollRectToVisible(bounds);
