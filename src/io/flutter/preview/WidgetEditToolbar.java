...
public class WidgetEditToolbar {
  ...
  EventStream<FlutterOutline> activeOutline;
  ...
  public WidgetEditToolbar(
    ...
    activeOutline.listen(this::activeOutlineChanged);
  }
  ...
  private void activeOutlineChanged(FlutterOutline outline) {
    synchronized (actionToChangeMap) {
      actionToChangeMap.clear();
    }
    ...
    if (selectionFile != null && outline != null) {
      ApplicationManager.getApplication().executeOnPooledThread(() -> {
        ...
        final int offset = converter.getConvertedOutlineOffset(outline);
        final int length = converter.getConvertedOutlineEnd(outline) - offset;
        ...
        final FlutterOutline newOutline = activeOutline.getValue();
        if (!Objects.equals(activeFile.getValue(), selectionFile) || !outline.equals(newOutline)) {
          return;
        }
  ...
  FlutterOutline getWidgetOutline() {
    final FlutterOutline outline = activeOutline.getValue();
    if (outline != null && outline.getDartElement() == null) {
      return outline;
    }
    return null;
  }
    ...
    ApplicationManager.getApplication().runWriteAction(() -> {
      try {
        ...
        if (hotReloadOnAction) {
          final InspectorService inspectorService = InspectorStateService.getInstance(project).getInspectorService();
  ...
  void createPopupMenu(Component comp, int x, int y) {
