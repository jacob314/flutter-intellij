...
/**
 * Class that handles displaying tooltips and custom logic for highlighting
 * selected nodes.
 */
    ...
    if (diagnostic != null && !diagnostic.isProperty()) {
      if (event.getClickCount() == 2) {
        if (panel.isSummaryTree) {
          ...
          panel.parentTree.applyNewSelection(panel.firstAncestorInParentTree(node), diagnostic, true);
        }
      }
    }
    event.consume();
