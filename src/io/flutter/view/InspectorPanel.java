...
public class InspectorPanel extends JPanel implements Disposable, InspectorService.InspectorServiceClient {
  ...
  public InspectorPanel(FlutterView flutterView,
    ...
    this(flutterView, flutterApp, inspectorService, isApplicable, treeType, false, null, isSummaryTree, legacyMode, shouldAutoHorizontalScroll);
  }
  ...
  public void onFlutterFrame() {
    ...
    if (!treeLoadStarted && myRootsTree == null) {
  ...
  public void updateSelectionFromService() {
    ...
    final CompletableFuture<DiagnosticsNode> pendingSelectionFuture = selectionGroups.getNext().getSelection(getSelectedDiagnostic(), treeType, isSummaryTree);
    ...
    CompletableFuture<?> selectionsReady = isSummaryTree ? CompletableFuture.allOf(pendingDetailsFuture, pendingSelectionFuture) : pendingSelectionFuture;
    selectionGroups.getNext().safeWhenComplete(selectionsReady, (ignored, error) -> {
      ...
      if (!legacyMode) {
        ...
        treeGroups.getCurrent().safeWhenComplete(treeGroups.getCurrent().getParentChain(newSelection), (ArrayList<DiagnosticsPathNode> path, Throwable ex) -> {
          if (ex != null) {
            LOG.error(ex);
            return;
          }
  ...
  private void animateTo(DefaultMutableTreeNode node) {
    ...
    // Backtrack to the the first non-property parent.
  ...
  private class PropertiesPanel extends TreeTableView implements DataProvider {
    ...
    public void showProperties(DiagnosticsNode diagnostic) {
      ...
      groups.cancelNext();
      groups.getNext().safeWhenComplete(diagnostic.getProperties(groups.getNext()), (ArrayList<DiagnosticsNode> properties, Throwable throwable) -> {
          if (throwable != null) {
            getTreeModel().setRoot(new DefaultMutableTreeNode());
