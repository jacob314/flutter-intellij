...
public class InspectorTreeUI extends BasicTreeUI {
  ...
  protected void paintVerticalPartOfLeg(final Graphics g, final Rectangle clipBounds, final Insets insets, final TreePath path) {
    ...
    if (node.getChildCount() < 2) {
      // We could optionally show lines for nodes with only 1 child but they
      // aren't needed.
      return;
    }
    ...
    if (lineX >= clipLeft && lineX <= clipRight) {
      ...
      for (int i = 0; i < node.getChildCount(); ++i) {
        ...
        DiagnosticsNode childDiagnostic = maybeGetDiagnostic(child);
        boolean dashed = false;
        if (childDiagnostic != null) {
          dashed = childDiagnostic.getStyle() == DiagnosticsTreeStyle.offstage;
        }

        Rectangle childBounds = getPathBounds(tree, path.pathByAddingChild(child));
        ...
        int bottom = Math.min(childBounds.y +
