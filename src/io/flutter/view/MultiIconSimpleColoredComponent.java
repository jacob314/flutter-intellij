 ...
 */
// TODO(jacobr): which license or both should we keep?
...
public class MultiIconSimpleColoredComponent extends JComponent implements Accessible, ColoredTextContainer {
  ...
  public int findFragmentAt(int x) {
    ...
    while (true) {
      ...
      while (j < myIcons.size() && myIcons.get(j).index <= i) {
        final int iconWidth =  myIcons.get(j).icon.getIconWidth() + myIconTextGap;
  ...
  protected int doPaintTextAndIcons(Graphics2D g, boolean focusAroundIcon) {
    ...
    while(true) {
      ...
      while (j < myIcons.size() && myIcons.get(j).index <= i) {
        final Icon icon = myIcons.get(j).icon;
